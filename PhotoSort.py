...
def main():
    ...
    layout = [
        ...
        [
            ...
            sg.InputText(key="-SOURCE-", default_text=working_directory),
            sg.FolderBrowse(initial_folder=working_directory),
        ],
        [
            sg.Text("Select destination folder"),
            sg.InputText(key="-DESTINATION-", default_text=f"{working_directory}"),
            sg.FolderBrowse(initial_folder=working_directory),
        ],
...
# Get graphic files including subfolders
...
# Return new name (absolute path) based on meta data
# YYYY/YYYYMM/YYYYMMDD/yyyymmdd_time.jpg
...
def copy_and_rename(src, dest):
    ...
    # If file name already exists, add number to the name
    unique_dest = uniquify(dest)

    copy2(src, unique_dest)


# Check if file with given name already exists, change name if needed
def uniquify(path):
